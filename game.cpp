...
void Game::run() {
	...
	case kResourceTypeAmiga:
		_res.load("FONT8", Resource::OT_FNT, "SPR.TNS");
		if (_res._isDemo) {
	...
	case kResourceTypeDOS:
		...
		_res._hasSeqData = _fs->exists("INTRO.SEQ.TNS");
	...
	switch (_res._type) {
	case kResourceTypeAmiga:
		_res.load("ICONE", Resource::OT_ICN, "SPR.TNS");
		_res.load("ICON", Resource::OT_ICN, "SPR.TNS");
...
void Game::displayTitleScreenAmiga() {
	static const char *FILENAME = "present.cmp.TNS";
	...
	if (_cut._id != 0xFFFF) {
		...
		if (_res._hasSeqData) {
			...
			if (SeqPlayer::_namesTable[_cut._id]) {
			        char name[16];
			        snprintf(name, sizeof(name), "%s.SEQ.TNS", SeqPlayer::_namesTable[_cut._id]);
				...
				if (p) {
					*p += num;
					...
					if (_cut._id == 0x3D) {
						playCutsceneSeq("CREDITS.SEQ.TNS");
