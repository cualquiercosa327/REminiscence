...
void Resource::load_FIB(const char *fileName) {
	...
	snprintf(_entryName, sizeof(_entryName), "%s.FIB.TNS", fileName);
	File f;
...
void Resource::load_MAP_menu(const char *fileName, uint8_t *dstPtr) {
	...
	snprintf(_entryName, sizeof(_entryName), "%s.MAP.TNS", fileName);
	File f;
	if (f.open(_entryName, "rb", _fs)) {
...
void Resource::load_PAL_menu(const char *fileName, uint8_t *dstPtr) {
	...
	snprintf(_entryName, sizeof(_entryName), "%s.PAL.TNS", fileName);
	File f;
	if (f.open(_entryName, "rb", _fs)) {
...
void Resource::load_SPR_OFF(const char *fileName, uint8_t *sprData) {
	...
	snprintf(_entryName, sizeof(_entryName), "%s.OFF.TNS", fileName);
	uint8_t *offData = 0;
	File f;
	if (f.open(_entryName, "rb", _fs)) {
...
void Resource::load_CINE() {
	...
	if (_type == kResourceTypeAmiga) {
		...
		if (_cine_txt == 0) {
			snprintf(_entryName, sizeof(_entryName), "%sCINE.TXT.TNS", prefix);
			File f;
			if (f.open(_entryName, "rb", _fs)) {
				const int len = f.size();
	...
	if (_cine_off == 0) {
		snprintf(_entryName, sizeof(_entryName), "%sCINE.BIN.TNS", prefix);
		File f;
		if (f.open(_entryName, "rb", _fs)) {
			int len = f.size();
...
void Resource::load(const char *objName, int objType, const char *ext) {
	...
	switch (objType) {
	case OT_MBK:
		snprintf(_entryName, sizeof(_entryName), "%s.MBK.TNS", objName);
		loadStub = &Resource::load_MBK;
		break;
	case OT_PGE:
		snprintf(_entryName, sizeof(_entryName), "%s.PGE.TNS", objName);
		loadStub = &Resource::load_PGE;
		break;
	case OT_PAL:
		snprintf(_entryName, sizeof(_entryName), "%s.PAL.TNS", objName);
		loadStub = &Resource::load_PAL;
		break;
	case OT_CT:
		snprintf(_entryName, sizeof(_entryName), "%s.CT.TNS", objName);
		loadStub = &Resource::load_CT;
		break;
	case OT_MAP:
		snprintf(_entryName, sizeof(_entryName), "%s.MAP.TNS", objName);
		loadStub = &Resource::load_MAP;
		break;
	case OT_SPC:
		snprintf(_entryName, sizeof(_entryName), "%s.SPC.TNS", objName);
		loadStub = &Resource::load_SPC;
		break;
	case OT_RP:
		snprintf(_entryName, sizeof(_entryName), "%s.RP.TNS", objName);
		loadStub = &Resource::load_RP;
		break;
	case OT_RPC:
		snprintf(_entryName, sizeof(_entryName), "%s.RPC.TNS", objName);
		loadStub = &Resource::load_RP;
		break;
	case OT_SPR:
		snprintf(_entryName, sizeof(_entryName), "%s.SPR.TNS", objName);
	...
	case OT_SPRM:
		...
		break;
	case OT_ICN:
		snprintf(_entryName, sizeof(_entryName), "%s.ICN.TNS", objName);
		loadStub = &Resource::load_ICN;
		break;
	case OT_FNT:
		snprintf(_entryName, sizeof(_entryName), "%s.FNT.TNS", objName);
		loadStub = &Resource::load_FNT;
		break;
	case OT_OBJ:
		snprintf(_entryName, sizeof(_entryName), "%s.OBJ.TNS", objName);
		loadStub = &Resource::load_OBJ;
		break;
	case OT_ANI:
		snprintf(_entryName, sizeof(_entryName), "%s.ANI.TNS", objName);
		loadStub = &Resource::load_ANI;
		break;
	case OT_TBN:
		snprintf(_entryName, sizeof(_entryName), "%s.TBN.TNS", objName);
		loadStub = &Resource::load_TBN;
		break;
	case OT_CMD:
		snprintf(_entryName, sizeof(_entryName), "%s.CMD.TNS", objName);
		loadStub = &Resource::load_CMD;
		break;
	case OT_POL:
		snprintf(_entryName, sizeof(_entryName), "%s.POL.TNS", objName);
		loadStub = &Resource::load_POL;
		break;
	case OT_CMP:
		snprintf(_entryName, sizeof(_entryName), "%s.CMP.TNS", objName);
		loadStub = &Resource::load_CMP;
		break;
	case OT_OBC:
		snprintf(_entryName, sizeof(_entryName), "%s.OBC.TNS", objName);
		loadStub = &Resource::load_OBC;
		break;
	case OT_SPL:
		snprintf(_entryName, sizeof(_entryName), "%s.SPL.TNS", objName);
		loadStub = &Resource::load_SPL;
		break;
	case OT_LEV:
		snprintf(_entryName, sizeof(_entryName), "%s.LEV.TNS", objName);
		loadStub = &Resource::load_LEV;
		break;
	case OT_SGD:
		snprintf(_entryName, sizeof(_entryName), "%s.SGD.TNS", objName);
		loadStub = &Resource::load_SGD;
		break;
	case OT_BNQ:
		snprintf(_entryName, sizeof(_entryName), "%s.BNQ.TNS", objName);
		loadStub = &Resource::load_BNQ;
		break;
	case OT_SPM:
		snprintf(_entryName, sizeof(_entryName), "%s.SPM.TNS", objName);
